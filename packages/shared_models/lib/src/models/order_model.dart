...
enum ItemOrder {
  ...
  const ItemOrder(this.displayName, this.color);

  factory ItemOrder.fromString(String status) {
    return ItemOrder.values.firstWhere(
      (e) => e.name == status,
      orElse: () => ItemOrder.pending, // Default value if not found
    );
  }
}
...
class Order {
  ...
  factory Order.fromMap(Map<String, dynamic> data, String id) {
    ...
    return Order(
      ...
      status: ItemOrder.fromString(data['status'] as String? ?? 'pending'),
